...
export async function startRelay(options: RelayOptions = {}): Promise<{ url: string; server: Server; store: AgentRoomStore }> {
  const port = options.port ?? 4317;
  if (port !== 0) await assertPortAvailable(port);
  ...
  const actualPort = await listen(server, port).catch((error: unknown) => {
    wss.close();
    server.close();
    throw error;
  });
  ...
  if (updatedRecord) await writeProjectLink(store.projectRoot, updatedRecord);
  return { url, server, store };
}

function attachSessionCookie(sessionToken: string, launchToken: string) {
...
function hasSessionToken(cookieHeader: string | undefined, token: string): boolean {
  return (cookieHeader ?? "")
    .split(";")
    .map((part) => part.trim())
...
function rejectExternalOrigins(req: express.Request, res: express.Response, next: express.NextFunction): void {
...
function rejectInvalidHost(req: express.Request, res: express.Response, next: express.NextFunction): void {
...
async function broadcastState(store: AgentRoomStore, wss?: WebSocketServer): Promise<void> {
  if (!wss) return;
