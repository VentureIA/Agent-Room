...
export async function setupAgentRoom(projectRoot = process.cwd(), input: SetupInput = {}): Promise<AgentRoomSetup> {
...
async function setupLinkedProject(projectRoot: string, input: SetupInput) {
...
async function resolveMcpCommand(): Promise<{ command: string; args: string[] }> {
  ...
  if (argvPath && /(?:^|\/)cli\.(?:js|ts)$/.test(argvPath) && (await isExistingPathInside(argvPath, packageRoot))) {
    ...
    if (argvPath.endsWith(".js")) return { command: process.execPath, args: [argvPath] };
  }

  const distCli = path.resolve(path.dirname(modulePath), "..", "cli.js");
  if (await exists(distCli)) return { command: process.execPath, args: [distCli] };
...
function renderAgentGuide(project: Project): string {
