...
function parseMcpClients(value: string, label: string): McpClientKind[] {
...
function printPixelBanner(mode: "init" | "join"): void {
  ...
  console.log("");
  for (const line of lines) console.log(`${color}${line}${reset}`);
  console.log("");
}

function shouldColorizeBanner(): boolean {
  return Boolean(process.stdout.isTTY && !process.env.NO_COLOR && !process.env.CI);
}

async function printReady(
  heading: string,
  results: Awaited<ReturnType<typeof installClients>>
): Promise<void> {
  const setup = results[0]?.setup;
  if (!setup) throw new Error("AgentRoom did not install any MCP client.");
  const link = await readProjectLink(process.cwd());
  ...
  if (link?.mode === "remote") {
    ...
    if (link.dashboardUrl) console.log(`Dashboard: ${link.dashboardUrl}`);
  }
...
function labelMcpClient(client: McpClientKind): string {
  return client === "claude" ? "Claude" : "Codex";
}

async function printMcpDoctor(projectRoot: string): Promise<void> {
...
): Promise<{ client: McpClientKind; ok: boolean; detail: string }> {
  const displayPath = path.relative(projectRoot, configPath) || configPath;
  try {
    const config = await readJson<Record<string, unknown>>(configPath);
    if (!config) return { client, ok: false, detail: `${displayPath} not found` };
    const servers = config[key];
    if (!isJsonObject(servers)) return { client, ok: false, detail: `${displayPath} has no ${key}.agentroom` };
    const agentroom = servers.agentroom;
    if (!isJsonObject(agentroom)) return { client, ok: false, detail: `${displayPath} has no ${key}.agentroom` };
