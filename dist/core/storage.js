...
export class AgentRoomStore {
    ...
    async readPermissionsMarkdownForProject(projectId) {
        ...
        if (project.path.startsWith("remote://"))
            return defaultPermissionsMarkdown();
    ...
    async writePermissionsMarkdownForProject(projectId, markdown) {
        const project = this.getProjectById(projectId);
        this.openDb()
            .prepare(`insert into project_permissions (project_id, markdown, updated_at)
         values (?, ?, ?)
         on conflict(project_id) do update set
        ...
        if (project.path.startsWith("remote://"))
            return `remote://${projectId}/permissions.md`;
        const permissionsPath = path.join(getProjectAgentRoomDir(project.path), "permissions.md");
        await writeTextFile(permissionsPath, markdown);
        return permissionsPath;
    }
    ...
    getProjectById(projectId) {
        const row = this.openDb().prepare("select * from projects where id = ? limit 1").get(projectId);
        if (!row)
            throw new Error(`Project does not exist in this AgentRoom: ${projectId}`);
        return mapProject(row);
    }
